...
SSORMethod::SSORMethod(const DenseMatrix& matrix, const std::vector<double>& b, double omega, int maxIterations)
...
std::vector<double> SSORMethod::solve() {
    ...
    for (int iteration = 0; iteration < maxIterations_; iteration++) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            solution[i] = (1.0 - omega_) * solution[i] + (omega_ / matrix_(i, i)) * (b_[i] - sum1 - sum2);
        }
